...
function funcionAgregarAlumno() {
  ...
  let nombre = removeAccents(
    document.getElementById("nombreInput").value.trim()
  );
  let apellido = removeAccents(
...
function mostrarListaDeAlumnos() {
  ...
  if (listaDeAlumnos.length === 0) {
    ...
    return;
  } // Recorre el array listaDeAlumnos y crea un li para cada uno
  else
    setTimeout(() => {
      mensajeCargando.style.display = "none";
      ...
      listaDeAlumnos.forEach((alumno, idx) => {
        ...
        li.innerText = `${
          idx + 1
        }) ${alumno.nombre.toUpperCase()} ${alumno.apellido.toUpperCase()} - Nota final: ${
          alumno.nota
        } `;
...
const mostrarAlumnosFiltradosBusqueda = (alumnosFiltrados) => {
  ...
  if (alumnosFiltrados.length === 0) {
    setTimeout(() => {
      mensajeCargando.style.display = "none";
      document.getElementById("p-lista").innerText =
      ...
      listaDeAlumnos.forEach((alumno, idx) => {
        ...
        botonEditar.addEventListener("click", () => {
          ...
          const nuevaNota = parseFloat(
            document.getElementById(`inputEditar-${idx}`).value
    ...
    timeoutId = setTimeout(() => {
      const resultados = listaDeAlumnos.filter((alumno) => {
        const nombreCompleto =
          `${alumno.nombre} ${alumno.apellido}`.toUpperCase();
